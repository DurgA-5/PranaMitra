...
const donorMenuItems = [
  ...
  { title: "My Availability", icon: Activity, path: "/donor/availability" },
  ...
  { title: "Lives Impacted", icon: HeartPulse, path: "/donor/lives-impacted" },
  { type: "divider", id: "div2" },
  { title: "Emergency Blood Request", icon: PlusCircle, path: "/donor/request-blood" },
  { title: "My Blood Requests", icon: ClipboardList, path: "/donor/my-requests" },
  { title: "Nearby Blood Banks", icon: MapPin, path: "/donor/blood-banks" },
      ...
      <aside className="w-72 h-full flex flex-col justify-between glass-sidebar text-white overflow-hidden shadow-2xl relative select-none">
        ...
        <div className="relative">
          ...
          <div className="h-20 flex items-center justify-between px-5 border-b border-white/5 shrink-0">
            <PranaMitraBrand size="sm" showSubtitle={true} subtitleText="Blood Management System" darkBg={true} />
            <button
              type="button"
              onClick={onClose}
          ...
          {/* Navigation */}
          <nav className="mt-4 px-4 space-y-1 overflow-y-auto max-h-[calc(100vh-160px)] scrollbar-thin pb-4">
            {menuItems.map((item, index) => {
              if (item.type === "divider") {
                return (
                  <div key={item.id || index} className="my-2.5 border-t border-white/10 opacity-60" />
                );
              }
