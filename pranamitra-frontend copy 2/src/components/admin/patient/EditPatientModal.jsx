...
function EditPatientModal({ patient, open, onClose, onUpdated }) {
  ...
  useEffect(() => {
    ...
    setFormData({
      ...
      requiredDate: patient.requiredDate ? patient.requiredDate.split("T")[0] : "",
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      onClose();
      toast.success("Patient request updated successfully.");
    } catch (error) {
      console.error("Failed to update patient", error);

      const data = error.response?.data;
      let errorMsg = "Unable to update patient request.";
      if (data) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
      <div className="flex h-full max-h-[85vh] w-full max-w-3xl flex-col rounded-2xl bg-white shadow-2xl animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5">
          <div>
            <h2 className="text-xl font-bold text-slate-800">Edit Patient Request</h2>
            <p className="mt-1 text-sm text-slate-500">
              Modify patient request details.
            </p>
          </div>
