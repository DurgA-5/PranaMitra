...
function EditBloodBankModal({ bank, open, onClose, onUpdated }) {
  ...
  useEffect(() => {
    ...
    setFormData({
      ...
      openingTime: bank.openingTime ? bank.openingTime.substring(0, 5) : "",
      closingTime: bank.closingTime ? bank.closingTime.substring(0, 5) : "",
      available24Hours: Boolean(bank.available24Hours),
    });
  }, [bank]);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      await bloodBankService.updateBloodBank(bank.id, payload);
      await onUpdated();
      ...
      let errorMsg = "Unable to update blood bank.";
      if (data) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
      <div className="flex h-full max-h-[85vh] w-full max-w-3xl flex-col rounded-2xl bg-white shadow-2xl animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5">
          <div>
            <h2 className="text-xl font-bold text-slate-800">Edit Blood Bank</h2>
            <p className="mt-1 text-sm text-slate-500">
              Update blood bank record details.
            </p>
          </div>
